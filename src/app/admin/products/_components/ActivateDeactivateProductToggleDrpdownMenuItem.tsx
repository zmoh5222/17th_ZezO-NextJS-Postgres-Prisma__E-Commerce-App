...
import ActivateDeactivateToggleServerAction from "../_actions/ActivateDeactivateToggleServerAction";
import { useTransition } from "react";
import { useRouter } from "next/navigation";
import toast from "react-hot-toast";
import { useFormStatus } from "react-dom";

export default function ActivateDeactivateProductToggleDrpdownMenuItem({
  id,
  ...
  return (
    <DropdownMenuItem
      ...
      onClick={async () => {
        ...
        const res = await ActivateDeactivateToggleServerAction(id, isAvailableForPurchase)
        if (res?.ok) {
          toast.success('toggle product done successfully', {
            duration: 3000,
            position: "bottom-center"
          })
        }
